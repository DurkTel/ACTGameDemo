...
public class XEventInfo<T> : IEventInfo
{
    public UnityAction<T> actions;

    public XEventInfo( UnityAction<T> action)
    {
        actions += action;
    }
}

public class XEventInfo : IEventInfo
{
    public UnityAction actions;

    public XEventInfo(UnityAction action)
    {
        actions += action;
    }
}
...
public class EventCenter : SingletonBase<EventCenter>
{
    ...
    public void AddEventListener<T>(EventDefine eventName, UnityAction<T> action)
    {
        ...
        {
            (eventDic[eventName] as XEventInfo<T>).actions += action;
        }
        //没有的情况
        else
        {
            eventDic.Add(eventName, new XEventInfo<T>( action ));
        }
    }
    ...
    public void AddEventListener(EventDefine eventName, UnityAction action)
    {
        ...
        if (eventDic.ContainsKey(eventName))
        {
            (eventDic[eventName] as XEventInfo).actions += action;
        }
        //没有的情况
        else
        {
            eventDic.Add(eventName, new XEventInfo(action));
        }
    }
    ...
    public void DispatchEvent<T>(EventDefine eventName, T info)
    {
        ...
        if (eventDic.ContainsKey(eventName))
        {
            if((eventDic[eventName] as XEventInfo<T>).actions != null)
                (eventDic[eventName] as XEventInfo<T>).actions.Invoke(info);
        }
    }
    ...
    public void DispatchEvent(EventDefine eventName)
    {
        ...
        if (eventDic.ContainsKey(eventName))
        {
            if ((eventDic[eventName] as XEventInfo).actions != null)
                (eventDic[eventName] as XEventInfo).actions.Invoke();
        }
    }
